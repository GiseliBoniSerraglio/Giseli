...
let subtracao = new Array();

document.write("<p>Subtração das matrizes</p>");
if (colunas1 === colunas2 && linhas1 === linhas2) {
    for (let i = 0; i < linhas1; i++) {
        subtracao[i] = []
        for (let j = 0; j < colunas1; j++) {
    ...
    for (let i = 0; i < matriz1.length; i++) {
        document.write("<tr>")
        for (let j = 0; j < matriz1[i].length; j++) {
            document.write("<td>" + subtracao[i][j] + "</td>")
        }
        document.write("</tr>")
    }
    document.write("</table><br>")
} else {
    document.write("<p>Não é possível subtrair as matrizes: elas têm tamanhos diferentes.</p><br>");
}